    ...
    public async Task<GenericResponseModel<bool>> Handle(UpdateAdminProfileCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var user = await _unitOfWork.Repository<User>()
                .FindBy(u => u.Id == currentUserId && !u.IsDeleted)
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(request.PhoneNumber))
                {
                    ...
                    if (trimmedPhone != user.PhoneNumber)
                    {
                        ...
                        var existingUserWithPhone = await _unitOfWork.Repository<User>()
                            .FindBy(u => u.PhoneNumber == trimmedPhone && u.Id != currentUserId && !u.IsDeleted)
                            .FirstOrDefaultAsync(cancellationToken);
            ...
            {
                await transaction.RollbackAsync(cancellationToken);
                _logger.LogError(ex, "Error updating admin profile for user {UserId}", currentUserId);
